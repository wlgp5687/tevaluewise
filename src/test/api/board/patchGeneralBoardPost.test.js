...
const patchGeneralBoardPostTest = () => {
	// 카페 일반 게시판 게시글 수정
	describe('PATCH /v1/boards/generals/:post_id', () => {
		...
		beforeAll(async () => {
			...
			const postResponse = await request.post(`/v1/boards/generals`).set(setData).send({
				cafe_id: 1,
				title: '글쓰기 제목',
				contents: '글쓰기 내용',
				is_notice: 'Y',
		...
		describe('board post owner patch', () => {
			...
			test('should return invalid status if cafe_id is empty', (done) => {
				...
				request.patch(`/v1/boards/generals/${postId}`).set(setData).send(sendData).expect(500, done);
			});
			...
			test('should return null if no title success patch board post', (done) => {
				...
				request
					.patch(`/v1/boards/generals/${postId}`)
					.set(setData)
					.send(sendData)
					.expect(200)
					.end((err, res) => {
...
export default patchGeneralBoardPostTest;
