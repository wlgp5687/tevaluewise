...
const getReferenceBoardPostsTest = () => {
	// 카페 일반 게시판 상세 조회
	describe('GET /v1/boards/reference', () => {
		...
		test('should return invalid status if page is not number', (done) => {
			...
			request
				.get(encodeURI(`/v1/boards/reference?page=${searchField.page}&lv1_id=${searchField.lv1_id}&cafe_id=${searchField.cafe_id}`))
				.set(setData)
				.expect(500, done);
		});
		...
		test('should return invalid status if limit is not number', (done) => {
			...
			request
				.get(encodeURI(`/v1/boards/reference?limit=${searchField.limit}&lv1_id=${searchField.lv1_id}&cafe_id=${searchField.cafe_id}`))
				.set(setData)
				.expect(500, done);
		});
		...
		test('should return invalid status if order is null', (done) => {
			...
			request
				.get(encodeURI(`/v1/boards/reference?order=${searchField.order}&lv1_id=${searchField.lv1_id}&cafe_id=${searchField.cafe_id}`))
				.set(setData)
				.expect(500, done);
		});
		...
		test('should return invalid status if lv1_id is not exist', (done) => {
			...
			request
				.get(encodeURI(`/v1/boards/reference?lv1_id=${searchField.lv1_id}&cafe_id=${searchField.cafe_id}`))
				.set(setData)
				.expect(500, done);
		});
		...
		test('should return posts if order is download_count searchField success get board posts', (done) => {
			searchField.order = 'download_count';
			request
				.get(encodeURI(`/v1/boards/reference?order=${searchField.order}&lv1_id=${searchField.lv1_id}&cafe_id=${searchField.cafe_id}`))
				.set(setData)
				.expect(200)
		...
		test('should return posts if search_keyword searchField success get board posts', (done) => {
			request
				.get(encodeURI(`/v1/boards/reference?search_keyword=${searchField.search_keyword}&lv1_id=${searchField.lv1_id}&cafe_id=${searchField.cafe_id}`))
				.set(setData)
				.expect(200)
				.end((err, res) => {
					if (err) return done(err);
		...
		test('should return posts if all searchField success get board posts', (done) => {
			request
				.get(
					encodeURI(
						`/v1/boards/reference?page=${searchField.page}&limit=${searchField.limit}&lv1_id=${searchField.lv1_id}&cafe_id=${searchField.cafe_id}&order=${searchField.order}&search_keyword=${searchField.search_keyword}`,
					),
				)
				.set(setData)
				.expect(200)
