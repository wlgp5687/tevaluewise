...
const deleteReferenceBoardPostTest = () => {
	// 카페 일반 게시판 게시글 삭제
	describe('Delete /v1/boards/reference/:post_id', () => {
		...
		describe('board post owner delete', () => {
			test('should return invalid status if postId is not number', (done) => {
				request
					.delete(`/v1/boards/reference/isNotNumber`)
					.set(setData)
					.expect(500, done);
			});
			...
			test('should return invalid status if postId is null', (done) => {
				request
					.delete(`/v1/boards/reference/${null}`)
					.set(setData)
					.expect(500, done);
			});
			...
			test('should return invalid status if postId is not exist', (done) => {
				request
					.delete(`/v1/boards/reference/9999999`)
					.set(setData)
					.expect(500, done);
			});
			...
			test('should return null if success delete board post', (done) => {
				request
					.delete(`/v1/boards/reference/${postId}`)
					.set(setData)
					.expect(200)
					.end((err, res) => {
...
export default deleteReferenceBoardPostTest;
